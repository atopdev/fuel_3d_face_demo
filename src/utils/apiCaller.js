...
const newAccessToken = async (email, refreshToken) => {
  try {
    ...
    localStorage.setItem('f3d_app_token', JSON.stringify(response.data));
...
export const authorizedRequest = async (method, path, options = {}) => {
  ...
  config.headers = { 'Content-Type': 'application/json' };
  let token = localStorage.getItem('f3d_app_token');
  const userEmail = localStorage.getItem('f3d_app_user_email');
  if (token && userEmail) {
